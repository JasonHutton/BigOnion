...
void GameWorldHelper::initTestScene(BOEngine* engine, Shader* shader)
{
	...
	// create suit man
	string osSuitMan;
	FileSystem::BuildOSPath(FileSystem::FindFile("game/assets/nanosuit/nanosuit.obj"), "game/assets/nanosuit/nanosuit.obj", osSuitMan);
	...
	suitMan->addComponent(new RenderComponent(engine, osSuitMan, shader)); // connect object - model
	...
	// create ground
	string osGround;
	FileSystem::BuildOSPath(FileSystem::FindFile("game/assets/ground/plane.obj"), "game/assets/ground/plane.obj", osGround);
	GameObject* ground = new GameObject("Ground");
	ground->transform.position = Vector3f(0, -3.25, 0);
	ground->transform.scale = Vector3f(100.0f, 1.0f, 100.0f);
	ground->addComponent(new RenderComponent(engine, osGround, shader));
